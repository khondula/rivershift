...
server <- function(input, output) {
    ...
    output$summary_table <- renderTable({
        readr::read_csv(file = glue::glue("{data_dir}/us-data/nwis_site_summaries/nwis-summary_{input$site_no}.csv"),
                        col_types = c("ciiincccccc")) %>%
            dplyr::select(srsname, parameter_nm, parameter_units, date_range_yrs, n_count, min_year, max_year)
    })
    
    site_data <- reactive({
        readr::read_csv(file = glue::glue("{data_dir}/us-data/nwis_site_data/nwis-data_{input$site_no}.csv"),
                        col_types = cols_only(
