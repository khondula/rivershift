...
Get information about sites and filter to just stream sites (258 sites)
...
data.frame(site_no = sites_all3_streams) %>% write_csv("/nfs/rivershift-data/us-data/sites_all3_streams.csv")
```
...
```{r read_nwis_qw}
# site_id <- sites_all3_streams[35] # for testing code in function
...
> readNWISqw (20 years beetwen 1980 and 2010; average values if anual visits <6; 
monthly/ daily values if anual visits > 6)
...
get_nwis_site_data <- function(site_id){
    ...
    readr::write_csv(nwis_data_site, 
                   glue::glue("{data_dir}/us-data/nwis_site_data_1990s/nwis-data_1990s_{site_id}.csv"))
    
  nwis_data_site20yr <- nwis_data_site %>% 
    group_by(parm_cd) %>% 
    summarise(n_count = n(),
            min_datetime = as.Date(min(startDateTime)),
            max_datetime = as.Date(max(startDateTime)),
  ...
  left_join(npc_codes_df, by = c("parm_cd" = "parameter_cd")) %>%
  arrange(-n_count) %>% 
    #filter(date_range_yrs > 20) %>%
    filter(min_year > 1980) %>%
    filter(max_year < 2010) %>%
        mutate(site_no = site_id) %>%
  dplyr::select(-min_datetime, -max_datetime, -date_range_interval)

  readr::write_csv(nwis_data_site20yr, 
                   glue::glue("{data_dir}/us-data/nwis_site_summaries_1990s/nwis-summary_1990s_{site_id}.csv"))
